...
export function countAllArrangements(input, fold = 1) {
    let result = 0;
    input.split('\n').map(line => line.trim().split(' ')).forEach(([pattern, sizesPart]) => {
        const sizes = sizesPart.split(',').map(Number);
        result += countMatches(repeat(pattern, fold), ...repeat(sizes, fold));
    });
    return result;
}

function repeat(arg, times) {
    if (typeof arg === 'string') {
        return new Array(times).fill(arg).join('?');
    }
    if (Array.isArray(arg)) {
        return new Array(times).fill(arg).flat();
    }
}
