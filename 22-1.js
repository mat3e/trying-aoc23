export function countRemovals(bricks) {
    const adjacency = findAdjacent(bricks);
    const groups = groupByZ(bricks);
    let result = 0;
    [...groups.values()].forEach((indicesSet) => {
        indicesSet.forEach((index) => {
            const colliding = adjacency.get(index);
            if (colliding.size === 0) {
                result++;
                console.info(toString(bricks[index]))
                return;
            }
            const allButCurrentCollisions = [...indicesSet].filter(i => i !== index).map(i => adjacency.get(i));
            // can be removed if all colliding are covered by other bricks
            if ([...colliding].every(i => allButCurrentCollisions.some(collisions => collisions.has(i)))) {
                result++;
                console.info(toString(bricks[index]))
            }
        });
...
function groupByZ(bricks) {
    const groups = new Map();
    let recentZ = bricks[0].z;
    groups.set(recentZ, new Set([0]));
    for (let i = 1; i < bricks.length; i++) {
        if (bricks[i].z === recentZ) {
            groups.get(recentZ).add(i);
            continue;
        }
        recentZ = bricks[i].z;
        groups.set(recentZ, new Set([i]));
    }
    return groups;
}
